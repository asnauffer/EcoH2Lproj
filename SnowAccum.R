...
SnowAccum<-function(Date, precip_mm, Tmax_C, Tmin_C, lat_deg, slope=0, aspect=0, tempHt=1, windHt=2, groundAlbedo=0.25,
                    SurfEmissiv=0.95, windSp=2, forest=0, startingSnowDepth_m=0, startingSnowDensity_kg_m3=450, logNoAccumWarm=TRUE){	
## Constants :
...
# 		SnowWaterEq[i] <- max(0,SnowWaterEq[i-1]-SnowMelt[i]+NewSnowWatEq[i])	# (m) Equiv depth of water
   	SnowWaterEq[i] <- max(0,SnowWaterEq[i-1]+NewSnowWatEq[i])	# (m) Equiv depth of water
	  
	}
	
	Results<-data.frame(Date, Tmax_C, Tmin_C, precip_mm, NewSnowWatEq*1000,SnowWaterEq*1000)
	colnames(Results)<-c("Date", "MaxT_C", "MinT_C", "Precip_mm", "SnowfallWatEq_mm","SnowWaterEq_mm")
	return(Results)
}	
